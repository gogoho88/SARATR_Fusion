...
# network backbone
backbone = 'vgg16'   # AConvnet, LMBNCNN, ESENet, Resnet
# network training
Num_epoch = 250
Learning_rate = 0.001
Batch_size = 128
...
X_train_cart = np.zeros((2,prepro_size,prepro_size,np.shape(X_train_tgt_cart)[2]), dtype='float')
X_test_cart = np.zeros((2,prepro_size,prepro_size,np.shape(X_test_tgt_cart)[2]), dtype='float')

# Cart crop
#tgt
for i in range(X_train_tgt_cart.shape[2]):
#    X_train_cart[0,:,:,i] = prepro_crop(X_train_tgt_cart[:,:,i], COM_train_tgt[:,i].astype('int'), [60,60])
    X_train_cart[0,:,:,i] = prepro_crop(X_train_tgt_cart[:,:,i], [64,64], [prepro_size,prepro_size])
for i in range(X_test_tgt_cart.shape[2]):
#    X_test_cart[0,:,:,i] = prepro_crop(X_test_tgt_cart[:,:,i], COM_test_tgt[:,i].astype('int'), [60,60])
    X_test_cart[0,:,:,i] = prepro_crop(X_test_tgt_cart[:,:,i], [64,64], [prepro_size,prepro_size])
#sdw
for i in range(X_train_tgt_cart.shape[2]):
    X_temp = X_train_sdw_cart[:,:,i]
    X_adj = np.zeros(X_temp.shape, dtype=float)
    X_adj[-96:,:] = X_temp[1:97,:] 
    X_adj[0:32,:] = X_temp[-32:,:]
    COM_sdw = COM_train_sdw[:,i].astype('int')
    COM_sdw[0] = COM_sdw[0]+32
    X_train_cart[1,:,:,i] = prepro_crop(X_adj, COM_sdw, [prepro_size,prepro_size])
for i in range(X_test_tgt_cart.shape[2]):
    X_temp = X_test_sdw_cart[:,:,i]
    X_adj = np.zeros(X_temp.shape, dtype=float)
    X_adj[-96:,:] = X_temp[1:97,:] 
    X_adj[0:32,:] = X_temp[-32:,:]
    COM_sdw = COM_test_sdw[:,i].astype('int')
    COM_sdw[0] = COM_sdw[0]+32
...
# =============================================================================
# In[5] Region-wise normalization & Custom dataset
# =============================================================================
...
if use_scaling==False:
    ...
    tgt_transform_train  = transforms.Compose([])
    tgt_transform_test  = transforms.Compose([])
    sdw_transform_train  = transforms.Compose([sub_dataaugmentation.Aug_RandomErasing(),
...
if backbone=='AConvnet':
    if mode=='fusion':
        model = sub_network_architectures.AConvNet_64_GRIF2(n_classes=len(list_target)).to(device)
    else:
    ...
    model = torchvision.models.vgg16(num_classes=len(list_target)).to(device)
