...
class LSoftmaxLinear(nn.Module):
    ...
    def find_k(self, cos):
        ...
        eps = 1e-7
        cos = torch.clamp(cos, -1 + eps, 1 - eps).clone()
        acos = cos.acos()
    ...
    def forward(self, input, target=None):
        if self.training:
            ...
            x, w = input.clone(), self.weight.clone()
            beta = max(self.beta, self.beta_min)
            logit = x.clone().mm(w.clone())
            indexes = range(logit.size(0))
            logit_target = logit[indexes, target].clone()

            # cos(theta) = w * x / ||w||*||x||
            w_target_norm = w[:, target].clone().norm(p=2, dim=0)
            x_norm = x.clone().norm(p=2, dim=1)
            cos_theta_target = logit_target.clone() / (w_target_norm * x_norm + 1e-10)

            # equation 7
            cos_m_theta_target = self.calculate_cos_m_theta(cos_theta_target.clone())

            # find k in equation 6
            k = self.find_k(cos_theta_target.clone())

            # f_y_i
            logit_target_updated = (w_target_norm *
                                    x_norm *
                                    (((-1) ** k * cos_m_theta_target) - 2 * k))
            logit_target_updated_beta = (logit_target_updated + beta * logit[indexes, target].clone()) / (1 + beta)

            logit[indexes, target] = logit_target_updated_beta.clone()
            self.beta *= self.scale
            return logit
        else:
            assert target is None
            return input.clone().mm(self.weight.clone())
